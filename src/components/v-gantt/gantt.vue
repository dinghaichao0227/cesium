    ...
    >
      <div v-show="!hideHeader" class="gantt-header" :style="{ width: `100%` }">
        ...
        <div class="gantt-header-timeline">
          <div ref="headerTimeline" class="gantt-header-timeline-container" :style="{ width: totalWidth + 'px' }">
            <timeline
              :start="start"
              :end="end"
              :cellWidth="cellWidth"
      ...
      <div class="gantt-body" :style="{ height: `calc(100% - ${actualHeaderHeight}px)` }">
        <div class="gantt-table">
          <div ref="marklineArea" :style="{ marginLeft: titleWidth + 'px' }" class="gantt-markline-area">
            <CurrentTime v-if="showCurrentTime" :getPositionOffset="getPositionOffset" />
            <mark-line
            ...
            >
              <template v-slot="{ timeConfig, getPosition }">
                <slot name="markLine" :timeConfig="timeConfig" :getPosition="getPosition"></slot>
              </template>
            </mark-line>
          </div>
          <div
                  ...
                  >
                    <template v-slot:blockItem="{ blockData }">
                      <TaskItem
                        ...
                        @mousedown.native.right.stop="handleRightClickBlock($event, rowData, blockData)"
                      />
                    </template>
                  </BlockRow>
                </template>
...
import { calcScalesAbout2Times, getBeginTimeOfTimeLine, isDayScale, scaleList } from '@/utils/timeLineUtils.js';
...
export default {
  ...
  data() {
    return {
      currentBlock: useCounterStore().currentBlock,
      currentRow: useCounterStore().currentRow,
      targetBlock: useCounterStore().targetBlock,
      targetRow: useCounterStore().targetRow,
      showRowList: useCounterStore().showRowList,
      cutRow: useCounterStore().cutRow,
      handleBlock: useCounterStore().handleBlock,
      handleRow: useCounterStore().handleRow,
      cutBlock: useCounterStore().cutBlock,

      setCurrentBlock: useCounterStore(),
      setCurrentRow: useCounterStore(),
      setCutBlock: useCounterStore(),
      setCutRow: useCounterStore(),
      setTargetBlock: useCounterStore(),
      setTargetRow: useCounterStore(),
      setHandleBlock: useCounterStore(),
      setHandleRow: useCounterStore(),
      //缓存节点
      selector: {
        gantt_leftbar: {},
        gantt_table: {},
  ...
  computed: {
    ...
    endTimeOfRenderArea() {
      ...
      const { beginTimeOfTimeLine, scrollLeft, cellWidth, scale, widthOfBlocksWrapper, totalWidth } = this;
  ...
  methods: {
    ...
    handleLeftClickBlock(event, rowData, blockItem) {
      this.setCurrentRow.setCurrentRow(rowData);
      this.setCurrentBlock.setCurrentBlock(blockItem);
    },
    handleRightClickBlock(event, rowData, blockItem) {
      this.setHandleRow.setHandleRow(rowData);
      this.setHandleBlock.setHandleBlock(blockItem);
    },
    handleRightClickRow(event, blockRow) {
      this.$refs.blockItemMenu.hide();
      this.setTargetBlock.setTargetBlock(null);
      this.setTargetRow.setTargetRow(blockRow);
    },
    moveCurrentBlock() {
      this.setCutBlock.setCutBlock(this.handleBlock);
      this.setCutRow.setCutRow(this.handleRow);
    },
    /*粘贴*/
    pasteBlock() {
      if (this.cutBlock) {
